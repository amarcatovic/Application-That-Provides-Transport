...
export default class VozacLinijaManager extends Component {
  ...
  ucitajRelacije = async () => {
    await fetch(
      `http://${getIP()}/publictransportcloud/api/Relacija/read.php?id=${
        this.state.data.prevoznik_id
      }`
    )
  ...
  ucitajVozacLinija = async () => {
    await fetch(
      `http://${getIP()}/publictransportcloud/api/Vozac/aktivna-linija.php?id=${
        this.state.data.id
      }`
    )
      .then((response) => response.json())
  ...
  ucitajStaniceRelacije = async () => {
    await fetch(
      `http://${getIP()}/publictransportcloud/api/RelacijaStanica/readNiz.php?id=${
        this.state.relacija_id
      }`
    )
      .then((response) => response.json())
      .then((data) => {
  ...
  updateSljedecuStanicu = async () => {
    fetch(
      `http://${getIP()}/publictransportcloud/api/Linija/sljedeca-stanica.php`,
      {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          id: this.state.linija_id,
          stanica_id: this.state.idStanica[this.state.stanicaIndex + 1],
        }),
      }
    )
      .then((response) => response.json())
  ...
  zavrsiLiniju = async () => {
    fetch(`http://${getIP()}/publictransportcloud/api/Linija/kraj.php`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  kreirajLiniju = async () => {
    fetch(`http://${getIP()}/publictransportcloud/api/Linija/create.php`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify({
        vozac_id: this.state.data.id,
        vozilo_id: this.state.vozilo_id,
        relacija_id: this.state.relacija_id,
        polazak: this.state.polazak,
        dolazak: this.state.dolazak,
      }),
    })
      .then((response) => response.json())
      .then((data) => {
  ...
  render() {
    if (this.state.loading && this.state.linija_id == null) {
      ...
      return (
        <View style={styles.container}>
          <Text>
            {this.state.data.id} {this.state.vozilo_id}
          </Text>
          <DatePicker
            ...
            renderItem={({ item }) => (
              ...
              >
                ...
                <View style={styles.textoviRelacija}>
                  ...
                  >
                    Relacija: {item.polaziste} - {item.odrediste}
                  </Text>
                  <Text style={styles.textRelacija}>
                    Tip Vozila: {item.vozilo}
                  </Text>
                  <Text style={styles.textRelacija}>
                    Interval: {item.interval}
                  </Text>
                  {this.state.relacija_id == item.id ? (
                    <Text style={styles.textRelacijaOdabrana}>Odabrana</Text>
                  ) : undefined}
                </View>
              </TouchableOpacity>
            )}
        ...
        return (
          <View style={styles.containerLinija}>
            <View style={styles.stanica}>
              ...
              >
                -{this.state.imenaStanica[this.state.stanicaIndex]}-
              </Text>
              <TouchableOpacity
                ...
                onPress={() => {
                  this.setState((prevState) => ({
                    stanicaIndex:
                      (prevState.stanicaIndex + 1) %
                      prevState.imenaStanica.length,
                  }));
